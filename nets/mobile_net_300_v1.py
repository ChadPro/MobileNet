...
def mobile_net(inputs, \
    ...
    assert bgr.get_shape().as_list()[1:] == [300, 300, 3]
    ...
    with tf.variable_scope(scope):
        ...
        with tf.variable_scope('part_2'):
            ...
            net, dw_5_5, pw_5_5 = separable_conv2d(net, [3, 3, 256, 1], [1, 1, 256, 256], is_training=is_training, scope='conv_5_5') #38*38

            net, dw_6, pw_6 = separable_conv2d(net, [3, 3, 256, 1], [1, 1, 256, 512], downsample=True, is_training=is_training, scope='conv_6') #19*19
            net, dw_7, pw_7 = separable_conv2d(net, [3, 3, 512, 1], [1, 1, 512, 1024], downsample=True, is_training=is_training, scope='conv_7') #10*10
            net, dw_8, pw_8 = separable_conv2d(net, [3, 3, 1024, 1], [1, 1, 1024, 1024], downsample=True, is_training=is_training, scope='conv_8') #5*5
            nets_dict['test'] = net
        '''
            3. Pool & FC
        '''
        ...
        # pool net
            net = tf.nn.avg_pool(net, (1, 5, 5, 1), (1, 5, 5, 1), padding='SAME', name='avg_pool')
